...
/* Repainted for a light terminal on the site's palette: one accent, one ink,
   and greys — rather than the shipped six-colour dark scheme. */
const tokenColors: Record<TokenType, string> = {
  command: "text-carbon font-semibold",
  flag: "text-signal-orange",
  string: "text-graphite/70",
  number: "text-graphite/70",
  operator: "text-graphite/40",
  path: "text-graphite/70",
  variable: "text-signal-orange",
  comment: "text-graphite/35",
  default: "text-carbon",
};
...
export function Terminal({
  ...
  useEffect(() => {
    ...
    /* Vendor component (@aceternity/terminal). The typing sequence is a phase
       machine, and this transition is the one step that has to commit the
       finished command before moving on. Left as shipped rather than
       restructured. */
  ...
  const prompt = (
    <span className="text-graphite/40">
      <span className="text-signal-orange">{username}</span>
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-xl border border-[#E8E8ED] bg-white">
        {/* Title Bar */}
        <div className="flex items-center gap-2 border-b border-[#E8E8ED] bg-[#FAFAFB] px-4 py-3">
          <div className="flex items-center gap-1.5">
            <div className="h-2.5 w-2.5 rounded-full bg-[#E4E4E9]" />
            <div className="h-2.5 w-2.5 rounded-full bg-[#E4E4E9]" />
            <div className="h-2.5 w-2.5 rounded-full bg-[#E4E4E9]" />
          </div>
        ...
        >
          {lines.map((line, i) => (
            <div key={i} className="leading-relaxed whitespace-pre-wrap">
              {line.type === "command" ? (
                ...
                <span className="text-graphite/60">{line.content}</span>
              )}
            </div>
          ))}
          ...
          {phase === "typing" && (
            <div className="leading-relaxed whitespace-pre-wrap">
              ...
              <span className="ml-0.5 inline-block h-4 w-2 bg-carbon/60 align-middle" />
            </div>
          )}
          ...
          {(phase === "done" ||
            ...
            <div className="leading-relaxed whitespace-pre-wrap">
              ...
              <span
                className={cn(
                  "inline-block h-4 w-2 bg-carbon/60 align-middle transition-opacity duration-100",
