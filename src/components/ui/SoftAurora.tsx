...
/**
 * SoftAurora — React Bits, the published TypeScript + Tailwind variant.
 * https://reactbits.dev · dependency: ogl
 *
 * Kept as published, with two changes:
 *
 ...
 *     the server.
 *   · `program` is a `const`, and the first `resize()` moved to after the
 *     canvas is appended. See the note at the call site.
 *
 * `lightMode` is in the source but not in the published props table. It is the
 * one this hero needs — it composites the aurora onto white as chroma rather
 * than emitting light onto black.
 */
...
export default function SoftAurora({
  ...
  useEffect(() => {
    ...
    /* Upstream declares `program` with `let`, calls `resize()` before the
       Program exists, and guards the uniform write with `if (program)`. That
       first call measures a container the canvas has not been appended to
       yet, so it is doing nothing useful; with `program` as a `const` it
       would also be a temporal-dead-zone throw. So the listener and the first
       measurement both move here, after the canvas is in the DOM — which is
       where the measurement is meaningful anyway. */
