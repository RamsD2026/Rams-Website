 ...
 * 04 — How it works.
 *
 * `TwinHow`'s shape, the settled one for a five- or six-step process on this
 * site: a rail across the top driving one evolving demonstration. The copy
 * column stays put; only the right side moves — the facility gains a layer and
 * its panel swaps.
 *
 * The facility is `TwinFacility` with its MHE routes running, because ATOS is
 * about work moving through a building and a still floor would say the
 * opposite. Its layers do not map one-to-one onto these five steps and are not
 * forced to: 03 and 04 share a layer, because orchestrating and assigning
 * change the plan rather than the floor. Inventing a layer to fill a slot
 * would be worse than reusing one honestly.
 *
...
const STEPS: {
...
}[] = [
  {
    n: "01",
    title: "Ingest",
    body: "Receive WMS, ERP, TMS, sensor or manually created operational requests.",
    detail: ["One queue, whatever the source", "Every task keeps its origin"],
    layer: 0,
    pane: { title: "Demand intake", chip: "Receiving", chipTint: "#3E63DD" },
    node: (t) => <PanelIngest key={t} />,
  },
  {
    n: "02",
    title: "Structure",
    body: "Apply configured templates to turn each order or event into executable steps.",
    detail: ["One order becomes many tasks", "Skills, assets and sequence set"],
    layer: 1,
    pane: { title: "Task templates", chip: "Building", chipTint: "#D95A00" },
    node: (t) => <PanelStructure t={t} />,
  },
  {
    n: "03",
    title: "Orchestrate",
    body: "Evaluate priority, due time, dependencies, location and resource requirements.",
    detail: ["Priority with a stated reason", "Constraints the site actually has"],
    layer: 3,
    pane: { title: "Sequenced plan", chip: "Planning", chipTint: "#D95A00" },
    node: () => <PanelOrchestrate />,
  },
  {
    n: "04",
    title: "Execute",
    body: "Recommend or dispatch tasks according to approved operating rules and permissions.",
    detail: ["Recommend first, dispatch later", "Permissions and approvals hold"],
    layer: 3,
    pane: { title: "Assignments", chip: "Dispatching", chipTint: "#16A34A" },
    node: () => <PanelExecute />,
  },
  {
    n: "05",
    title: "Adapt",
    body: "Respond to delays, exceptions, completions and resource changes while preserving traceability.",
    detail: ["A new plan version, not a patch", "Every change attributable"],
    layer: 5,
    pane: { title: "Replan", chip: "v19", chipTint: "#16A34A" },
    node: () => <PanelAdapt />,
  },
];

export function AtsHow() {
  ...
  return (
    <Section surface="white" id="how">
      <SectionHeader
        ...
        top="Demand in."
        bottom="Coordinated work out."
        size="compact"
        width="wide"
        body="ATOS creates a governed loop from incoming work to completed operational outcome."
      />
