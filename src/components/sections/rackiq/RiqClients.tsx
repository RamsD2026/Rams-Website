 ...
 *
 * ── The dark treatment ──────────────────────────────────────────────
 * Every file in `public/clients/` is a transparent PNG in the mark's real
 * colours, which is right on a light ground and useless here: Caterpillar's
 * black and Ferrero's brown vanish against #0E0E0F.
 *
 * This strip used to answer that with `filter: brightness(0) invert(1)`,
 * which paints every opaque pixel white. It worked for plain wordmarks and
 * destroyed every mark with white letters on a coloured fill — JCB, Volvo,
 * Flipkart's bag — because the fill and the letters became the same white
 * and the logo turned into a solid block.
 *
 * So the strip reads `public/clients-dark/` instead, generated by
 * `scripts/gen-dark-clients.mjs`: every mark in one flat white, no grey
 * mid-tones, with the lighter parts inside it cut out — JCB's letters through
 * a solid badge, Volvo's through its band — the way a brand prints its own
 * single-colour reversal. The colour originals in `public/clients/` are
 * untouched. No filter at runtime. Held at 0.5 and coming up to 0.9 on
 * hover, as before.
 *
