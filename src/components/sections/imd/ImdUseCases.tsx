...
const CASES = [
  {
    icon: TrendingDown,
    kicker: "Breakdown prevention",
    k: "Investigate emerging conditions",
    body: "Surface supported diagnostic changes before they become operationally disruptive failures.",
  },
  {
    icon: BatteryCharging,
    kicker: "Battery management",
    k: "Improve charging decisions",
    body: "Compare battery condition, charging history and equipment workload to plan service or replacement.",
  },
  {
    icon: CalendarClock,
    kicker: "Preventive maintenance",
    k: "Service by use and condition",
    body: "Plan intervention using operating hours, condition, history and the maintenance window available.",
  },
  {
    icon: ShieldAlert,
    kicker: "Impact-led inspection",
    k: "Connect safety events to MHE health",
    body: "Use RTSS impact context to initiate an inspection and preserve the resulting diagnostic record.",
  },
  {
    icon: Wrench,
    kicker: "Vendor + workshop control",
    k: "Track external maintenance",
    body: "Keep complaint, quotation, work order, parts, service notes and verification connected to the asset.",
  },
  {
    icon: Scale,
    kicker: "Repair or replace",
    k: "Use lifecycle evidence",
    body: "Compare reliability, downtime, maintenance cost and utilisation before a capital decision.",
  },
];

export function ImdUseCases() {
  ...
  return (
    <Section surface="white" id="use-cases">
      <style>{`
        @property --imduse-shine-angle {
          syntax: '<angle>';
          initial-value: 0deg;
          inherits: false;
        }
        .imduse-card { position: relative; isolation: isolate; }
        .imduse-card::before {
          content: "";
          position: absolute;
          inset: -1px;
        ...
        .imduse-card:hover::before {
          opacity: 1;
          animation: imduse-shine 2.4s linear infinite;
        }
        @keyframes imduse-shine {
          to { --imduse-shine-angle: 360deg; }
        }
        @media (prefers-reduced-motion: reduce) {
          .imduse-card:hover::before { animation: none; }
        }
      `}</style>

      <SectionHeader
        eyebrow="Key use cases"
        top="Manage the complete"
        bottom="MHE health programme."
        size="compact"
        width="wide"
        body="IMDS supports daily condition visibility, workshop planning and long-term fleet decisions."
      />
        ...
        >
          {CASES.map((c) => (
            <div key={c.kicker} style={{ width: cardWidth, flexShrink: 0 }}>
              <article
                className="imduse-card group flex flex-col h-full p-7 sm:p-8 bg-white transition-transform duration-300 hover:-translate-y-1"
                style={{
                  borderRadius: 12,
